import { BezierSegment, Corner, Point, Space, Spot, TrackData, TrackMetadata } from '@/types/spline';
import { calculateChainArcLength, calculateChainTangent, evaluateChainAtT, findTForDistance } from './bezierChain';
...
export function calculateCurvature(
  ...
  const secondDeriv = {
    x: 6 * mt * p0.x - 12 * mt * cp1.x + 6 * mt * cp2.x + 6 * t * cp1.x - 12 * t * cp2.x + 6 * t * p1.x,
    y: 6 * mt * p0.y - 12 * mt * cp1.y + 6 * mt * cp2.y + 6 * t * cp1.y - 12 * t * cp2.y + 6 * t * p1.y,
  };
...
export function discretizePathToSpaces(
  ...
  for (let spaceIndex = 0; spaceIndex < targetSpacesPerLap; spaceIndex++) {
    ...
    const curvature = calculateCurvatureAtChainPosition(bezierSegments, segmentIndex, t);
    
    // Generate spots for this space
    const spots = generateSpotsForSpace(position, tangent, trackWidth, spotCount);
...
export function autoSuggestCorners(
  ...
  for (const { space, curvature } of highCurvatureSpaces) {
    ...
    const tooClose = Array.from(usedIndices).some(usedIndex => 
      Math.abs(space.index - usedIndex) < minCornerSpacing
    );
 ...
 */
function determineCornerType(curvature: number): 'slow' | 'medium' | 'fast' | 'chicane' {
 ...
 */
function calculateSpeedLimit(curvature: number, cornerType: 'slow' | 'medium' | 'fast' | 'chicane'): number {
  switch (cornerType) {
    case 'slow': return Math.max(1, Math.floor(6 - curvature * 20));
    case 'medium': return Math.max(2, Math.floor(6 - curvature * 15));
    case 'fast': return Math.max(3, Math.floor(6 - curvature * 10));
    case 'chicane': return Math.max(2, Math.floor(6 - curvature * 12));
    default: return 3;
  }
}
 ...
 */
function calculateCornerDifficulty(curvature: number, speedLimit: number): number {
...
export function updateTrackMetadata(
  ...
  const averageCurvature = spaces.reduce((sum, space) => sum + space.metadata.curvature, 0) / spaces.length;
 ...
 */
function calculateTrackDifficulty(corners: Corner[], averageCurvature: number): number {
  if (corners.length === 0) return Math.min(10, Math.max(1, averageCurvature * 20));
  
  const averageCornerDifficulty = corners.reduce((sum, corner) => sum + corner.difficulty, 0) / corners.length;
...
export function validateTrackData(trackData: TrackData): string[] {
  ...
  for (const corner of trackData.corners) {
    ...
    if (corner.speedLimit < 1 || corner.speedLimit > 6) {
      errors.push(`Corner at space ${corner.spaceIndex} has invalid speed limit: ${corner.speedLimit}`);
    }
  }
  ...
  for (const space of trackData.spaces) {
    ...
    const raceLineSpots = space.spots.filter(spot => spot.type === 'race-line');
  ...
  const hasSelfIntersections = checkForSelfIntersections(trackData.splinePath.segments);
 ...
 */
function calculateBoundingBox(segment: BezierSegment): { minX: number; maxX: number; minY: number; maxY: number } {
  const points = [segment.startPoint, segment.cp1, segment.cp2, segment.endPoint];
