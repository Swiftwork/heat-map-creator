import { TrackData, TrackExportData, TrackValidationResult } from '@/types/spline';
 ...
 */
export function importTrackData(data: string, format: 'json' | 'heat-track' | 'legacy'): TrackData {
  try {
    ...
    throw new Error(`Failed to parse track data: ${error instanceof Error ? error.message : 'Unknown error'}`);
  }
}
...
function importLegacyFormat(legacyData: any): TrackData {
  ...
  const trackData: TrackData = {
    ...
    discretizationSettings: legacyData.discretizationSettings || createDefaultDiscretizationSettings(),
    validationErrors: [],
    isValid: false,
    lastValidated: new Date().toISOString(),
  };
 ...
 */
export function validateTrackDataComprehensive(trackData: TrackData): TrackValidationResult {
  ...
  if (trackData.corners.length === 0) {
    warnings.push('No corners defined - consider adding corners for gameplay variety');
  }
  
  if (trackData.spaces.length < 20) {
    warnings.push('Track has fewer than 20 spaces - consider increasing discretization');
  }
  
  if (trackData.spaces.length > 100) {
    warnings.push('Track has more than 100 spaces - consider reducing discretization');
  }
  
  // Generate suggestions
  if (trackData.metadata.boardMetadata.difficulty < 3) {
    suggestions.push('Track difficulty is low - consider adding more challenging corners');
  }
  
  if (trackData.metadata.boardMetadata.difficulty > 8) {
    suggestions.push('Track difficulty is high - consider adding more straight sections');
  }
  ...
  const gameplayValidation = {
    cornerPlacementValid: trackData.corners.every(corner => 
      corner.spaceIndex < trackData.spaces.length
    ),
    startFinishValid: trackData.metadata.startFinishSpaceIndex < trackData.spaces.length,
